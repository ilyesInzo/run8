	...
	getPrices : function(cmp, event, index){
        ...
        action.setCallback(this, function(response) {
            ...
            if (index == 0) {
                ...
                this.setPriceLines(cmp, "v.sleeve", 'Sleeve', _response.details);
            ...
            cmp.set("v.totalSpecificMOQ",_response.totalSpecificMOQ);
 			...
 			this.descprice(cmp, "v.printing", "v.printing_desc_price"); 
	...
	setPrices : function(cmp, attr, attrName, _response){
    	...
    	for(var i = 0 ; i < _sectionLen ; i++){
    		...
    		if(_response[attrName] && _response[attrName][_title]){
                ...
                _section[i].x10 = (_response[attrName][_title][0] ? _response[attrName][_title][0].List_Price_Display__c  : '');
        		_section[i].x15 = (_response[attrName][_title][1] ? _response[attrName][_title][1].List_Price_Display__c  : '');
        		_section[i].x20 = (_response[attrName][_title][2] ? _response[attrName][_title][2].List_Price_Display__c  : '');
        		_section[i].x25 = (_response[attrName][_title][3] ? _response[attrName][_title][3].List_Price_Display__c  : '');
        		_section[i].x50 = (_response[attrName][_title][4] ? _response[attrName][_title][4].List_Price_Display__c  : '');
        		_section[i].x75 = (_response[attrName][_title][5] ? _response[attrName][_title][5].List_Price_Display__c  : '');
        		_section[i].x100 = (_response[attrName][_title][6] ? _response[attrName][_title][6].List_Price_Display__c  : '');
        		_section[i].x200 = (_response[attrName][_title][7] ? _response[attrName][_title][7].List_Price_Display__c  : '');
        		_section[i].x300 = (_response[attrName][_title][8] ? _response[attrName][_title][8].List_Price_Display__c  : '');
        		_section[i].x500 = (_response[attrName][_title][9] ? _response[attrName][_title][9].List_Price_Display__c  : '');
                _section[i].x1000 = (_response[attrName][_title][10] ? _response[attrName][_title][10].List_Price_Display__c  : '');
               // _section[i].xSpecific = (_response[attrName][_title][11] ? _response[attrName][_title][11].List_Price_Display__c : '');
...
/*                _section[i].x10 = (_response[attrName][_title][0] ? _response[attrName][_title][0].List_Price__c : '');
		       ...
		       // _section[i].xSpecificOverride = (_response[attrName][_title][11] ? _response[attrName][_title][11].Price_Override__c : '');
	...
	descprice : function(cmp, item, disp){
		...
		for(var i = 0 ; i < itemlst.length ; i++){
			...
			//overrideprice += (isNaN(itemlst[i].xSpecificOverride))?0:itemlst[i].xSpecificOverride;
		}
        console.log('DESCPRICE-----------------END');        
		var display = (overrideprice > 0)?"":"none";
		cmp.set(disp, display);
	},
	savepricehelper : function(cmp, section, listitems, updatelistitems){
		console.log('savepricehelper ==============');	
    	...
    	console.log(listitems[i]);	
    		...
    		console.log('----ovrrridepricee-----');
    		console.log(listitems[i].x500Override)	;
    	} 
