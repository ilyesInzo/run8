({
	handleEvent : function(cmp, event, helper) {
		...
		if(name == "Printing_technologies__c" || name == "Diameter_mm__c" || name == "Head_Name__c" || name == "Dispensing_System_Family__c"){
		
			var action = cmp.get("c.getPlasticTubeEUColors");
	...
	refreshmessage : function(cmp, event, helper) {
		...
		helper.firecolorvalues(cmp, event, "NB_Flexo_Colors__c", cmp.get("v.selectedconfig").NB_Flexo_Colors__c);
