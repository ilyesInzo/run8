({
	...
	getPrices : function(cmp, event){
        ...
        action.setCallback(this, function(response) {
        	...
        	this.setPrices(cmp, "v.upperpartDeco", 'Upper Part Decoration', _response.details);            
        	this.setPrices(cmp, "v.bottleUpcharge", 'Bottle Upcharges', _response.details);
            this.setPrices(cmp, "v.pumpOptions", 'Pump Options', _response.details);
            ...
            cmp.set("v.currency", _response.currency);  
 			...
 			this.descprice(cmp, "v.upperpartDeco", "v.upper_part_Decoration_desc_price");             
 			this.descprice(cmp, "v.bottleUpcharge", "v.bottle_upcharge_desc_price");
            this.descprice(cmp, "v.pumpOptions", "v.pump_options_desc_price");
	...
	setPrices : function(cmp, attr, attrName, _response){
    	...
    	for(var i = 0 ; i < _sectionLen ; i++){
    		...
    		if(_response[attrName] && _response[attrName][_title]){
        		...
        		_section[i].x50 = (_response[attrName][_title][1] ? _response[attrName][_title][1].List_Price__c : '');
        		_section[i].x100 = (_response[attrName][_title][2] ? _response[attrName][_title][2].List_Price__c : '');
        		_section[i].x250 = (_response[attrName][_title][3] ? _response[attrName][_title][3].List_Price__c : '');
                _section[i].x10Override = (_response[attrName][_title][0] ? _response[attrName][_title][0].Price_Override__c : '');
        		_section[i].x50Override = (_response[attrName][_title][1] ? _response[attrName][_title][1].Price_Override__c : '');
        		_section[i].x100Override = (_response[attrName][_title][2] ? _response[attrName][_title][2].Price_Override__c : '');
         		_section[i].x250Override = (_response[attrName][_title][3] ? _response[attrName][_title][3].Price_Override__c : '');
		    }
